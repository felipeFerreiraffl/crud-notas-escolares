...
import { getById } from '../../service/api/api';
...
export default function ProfInfos() {
  ...
  useEffect(() => {
    const fetchInfos = async () => {
      try {
        const response = await getById(user.id, 'professor');
  ...
  const handleSave = async () => {
    try {
      await updateObj(user.id, 'professor', info);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.infosContainer}>
        ...
        <InfosField
          ...
          info={info.areaEnsino}
          editable={editMode}
          onChange={(text) => setInfo({ ...info, areaEnsino: text })}
          backgroundColor={editMode ? ntColors.ntWhite : ntColors.ntGray}
        />
      </View>
      <Image
